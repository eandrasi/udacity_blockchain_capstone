...
contract Ownable {
    ...
    function getOwner() public returns(address _theOwner){
        _theOwner = _owner;
    }
    ...
    event NewOwner(address _theOwner);
}
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(_paused == false, "Contract is now PAUSED!");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, "Can not self-approve!");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "Require _checkOnERC721Received");
    }
...
contract ERC721Enumerable is ERC165, ERC721 {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        require(index < balanceOf(owner), "Must own token!");
    ...
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "Index to high!");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    // function getBaseTokenURI() public returns (string memory baseTokenURI) {
    //     baseTokenURI = _baseTokenURI;
    // }

    //
    //
    //SHould come back and refactor!!!!!!!!!!!!
    //
    //
    ...
    function baseTokenURI() public returns (string memory baseTokenURI) {
        baseTokenURI = _baseTokenURI;
    }
...
contract EACustomERC721Token is ERC721Metadata ("EACapstone", "EAC", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){

    function mint(address to, uint tokenId, string memory tokenURI)
        public
        onlyOwner
        returns (bool){
            super._mint(to, tokenId);
